		 ...
		 try {
			...
			// Create the model:
			 ...
			 //Create covering/partitioning constraints and objective function
				...
				GRBLinExpr[] relXandZ_constraints = new GRBLinExpr[nSatellites];
			...
			// Initializes the constraints:
				...
				for(int i = 0;i<nSatellites; i++) {
					...
					relXandZ_constraints[i] = new GRBLinExpr();
				}

			// Create variables:
			
				for(int s = 1;s <= nSatellites;s++) {
					model.addVar(0, 1, data.getSatellites_opening().get(s-1), GRB.BINARY, ("z_"+s));
				}
			...
			for(RoutePool pool : pools) {	
				if(pool.getSatellite() == 0) { // FE
					...
					while(iterator.hasNext()) {
						...
						model.addVar(0, 1, (double)r.getAttribute(RouteAttribute.COST) + data.getFixed_cost1(),GRB.BINARY, "x_"+counter_fe);
			...
			for(RoutePool pool : newPools) {	
				...
				while(iterator.hasNext()) {
					...
					model.addVar(0, 1, (double)r.getAttribute(RouteAttribute.COST)+ data.getFixed_cost2(),GRB.BINARY, "y_"+counter_se);
			...
			for(int s = 1;s <= nSatellites;s++) {
				objectiveExpr.addTerm(data.getSatellites_opening().get(s-1), model.getVarByName("z_"+s));
				relXandZ_constraints[s-1].addTerm(-1,model.getVarByName("z_"+s));
				maxSE_ctr[s-1].addTerm(-1 * data.getSatellites_capacities().get(s-1), model.getVarByName("z_"+s));
			}	
			...
			for(RoutePool pool : pools) {
				...
				if(pool.getSatellite() == 0) { // FE
					...
					while(iterator.hasNext()) {
						...
						objectiveExpr.addTerm(((double)r.getAttribute(RouteAttribute.COST)+ data.getFixed_cost1()), model.getVarByName("x_"+counter_fe));
						...
						// Update FE capacity constraints constraints:
							...
							for(int i=start;i<end;i++){
								...
								// Updates the variables relation constraints:
								
								relXandZ_constraints[route.get(i)-1].addTerm(1,model.getVarByName("x_"+counter_fe));
								
								// Capacity of the satellites:
								
								maxSE_ctr[route.get(i)-1].addTerm(1,model.getVarByName("f_"+counter_fe+"-"+route.get(i)));
			...
			for(RoutePool pool : newPools) {
				...
				while(iterator.hasNext()) {
					...
					// Updates the objective function:
						...
						objectiveExpr.addTerm(((double)r.getAttribute(RouteAttribute.COST)+ data.getFixed_cost2()), model.getVarByName("y_"+counter_se));
			...
			for(int i = 0;i<nSatellites; i++) {
				model.addConstr(satFlow_ctr[i],GRB.EQUAL,0,"FlowAtSatellite_"+(i+1));
				model.addConstr(maxSE_ctr[i], GRB.LESS_EQUAL, 0, "CapacitySatellite_"+(i+1));
				model.addConstr(relXandZ_constraints[i],GRB.EQUAL,0,"RelXandYSatellite_"+(i+1));
			}
