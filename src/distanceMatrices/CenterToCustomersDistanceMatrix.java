 ...
 */
public class CenterToCustomersDistanceMatrix extends ArrayDistanceMatrix{
	...
	public CenterToCustomersDistanceMatrix(String path) throws IOException {
		...
		// Read the coordinates of the nodes:
			...
			if(path.contains("coord")) { //Prodhon instances
				...
				for(int i = 1 ; i < nbCustomers+1; i++) {
					attrs = line.split("	");
					coorx += Double.parseDouble(attrs[0].trim());
					coory += Double.parseDouble(attrs[1].trim());
			...
			xCoors.add(0, coorx / xCoors.size());
			yCoors.add(0, coory / yCoors.size());
...
if(path.contains("coord")) { //Prodhon instances
				...
				for(int i = 0; i < dimension; i++) {
					...
					for(int j = 0; j < dimension; j++) {
						...
						distances[i][j] = Math.ceil(euc.calc(xCoors.get(i),yCoors.get(i),xCoors.get(j),yCoors.get(j)) * 100);
				...
				for(int i = 0; i < dimension; i++) {
					...
					for(int j = 0; j < dimension; j++) {
						...
						distances[i][j] = Math.ceil(euc.calc(xCoors.get(i),yCoors.get(i),xCoors.get(j),yCoors.get(j)) * 10);
