	...
	public void buildImprovementMILP( ArrayList<RoutePool> fePools, ArrayList<RoutePool> sePools, Hashtable<Integer,Cluster> clusters,Hashtable<Integer,ArrayList<Integer>> clusters_of_route,Hashtable<Integer,ArrayList<String>> insertionCostsAndPositions_to_route,Hashtable<Integer,Hashtable<Integer,ArrayList<Double>>> removalCosts_to_route) throws IloException {
		...
		// Create the variables:
			...
			// Initialize hashtable to store the variables:
				...
				Hashtable<Integer,IloNumVar> z_s = new Hashtable<Integer,IloNumVar>(); //Do we open/use the satellite ?
			...
			//Populate the hashtables:
				...
				// Open / use satellite variables:
				
					for(int i = 1; i <= nSatellites; i++) {
						
						z_s.put(i, model.boolVar("zs_"+i));
		...
		// Initializes the constraints:
			...
			// Relationship between opening a satellite and using a FE route:
			...
			// Update the objective function and the relXandZ constraints:
				
				for(int s = 1;s <= nSatellites;s++) {
					of.addTerm(data.getSatellites_opening().get(s-1), z_s.get(s));
					relXandZ_constraints[s-1].addTerm(-1,z_s.get(s));
				}
		...
		// Fill the constraints and update the obj function:
				...
				for(RoutePool pool : fePools) {
					...
					while(iterator.hasNext()) {
						...
						// Update the objective function:
							...
							of.addTerm((double)r.getAttribute(RouteAttribute.COST) + data.getFixed_cost1(),x_r.get(counter_fe));
						...
						// Iterate over the satellites in the route:
							...
							for(int i=start;i<end;i++){
								...
								// Update rel X and Z constraints:
								
									relXandZ_constraints[route.get(i)-1].addTerm(1,x_r.get(counter_fe));
								...
								// Max satellites 
									...
									maxSE_constraints[route.get(i)-1].addTerm(1,f_rs.get(counter_fe+" - "+route.get(i)));
				...
				for(RoutePool pool : sePools) {
					...
					while(iterator.hasNext()) {
						...
						// Update the objective function: y_r variables
							...
							of.addTerm((double)r.getAttribute(RouteAttribute.COST) + data.getFixed_cost2(),y_r.get(counter_se));
			...
			//Add constraints to the model
				...
				// Original constraints + SE capacity
					...
					for(int i = 0;i<nSatellites; i++) {
						model.addGe(data.getSatellites_capacities().get(i),maxSE_constraints[i],"CapacitySatellite_"+(i+1));
					}
					
					for(int i = 0;i<nSatellites; i++) {
						model.addEq(0,relXandZ_constraints[i],"RelXandYSatellite_"+(i+1));
					}
